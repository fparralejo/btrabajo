...
<h3 class="page-header">Listado Seguimiento Oferta Nº</h3>
...
//	}
	...
	function leerSeguimiento(id_oferta){
        $.ajax({
	...
	function borrarSeguimiento(id_seguimiento){
            if (confirm("¿Desea borrar el seguimiento?"))
            {
                $.ajax({
                  data:{"id_seguimiento":id_seguimiento},  
                  url: '{{ URL::asset("seguimiento/delete") }}',
                  type:"get",
                  success: function(data) {
                ...
                setTimeout(function ()
                {
                    document.location.href="{{URL::to('seguimiento')}}";
                }, 1000);
            }
	}
...
<table id="ejemplo1" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
    ...
    $url="javascript:leerSeguimiento('$seguimiento->id_seguimiento');";
    ?>
        <tr>
            ...
            <td>
                <button type="button" onclick="borrarSeguimiento({{ $seguimiento->id_seguimiento }})" class="btn btn-xs btn-danger">Borrar</button>
            </td>
        </tr>
    @endforeach
    </tbody>
